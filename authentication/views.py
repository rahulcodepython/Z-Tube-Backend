    ...
    def get(self, request):
        try:
            return (
                ...
                if check_authenticated_user(request.user)
    ...
    def post(self, request):
        try:
            ...
            if not serialized_data.is_valid():
                return response.Response(
                    {"error": str(serialized_data.errors)},
                    status=status.HTTP_406_NOT_ACCEPTABLE,
                )
...
class CreateJWTView(views.APIView):
    def post(self, request):
        try:
            ...
            user = authenticate(
                username=User.objects.get(email=given_email).username, password=password
            )
            ...
            if not user.is_active:
                return response.Response(
                    {"error": "You are not verified yet. Verify first."},
                    status=status.HTTP_406_NOT_ACCEPTABLE,
                )
...
class ResetUserPasswordView(views.APIView):
    ...
    def get(self, request):
        try:
            ...
            email.reset_email_confirmation(
    ...
    def post(self, request):
        try:
            ...
            if not models.ResetPasswordCode.objects.filter(
                ...
                return response.Response(
                    {"error": "You have entered wrong code. Try again."},
                    status=status.HTTP_406_NOT_ACCEPTABLE,
                )
...
class UserPeekView(views.APIView):
    ...
    def get(self, request):
        try:
            return response.Response(
                serializers.UserPeekSerializer(request.user).data,
                status=status.HTTP_200_OK,
            )
